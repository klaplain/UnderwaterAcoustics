...
int main(void)
{
	...
	{
		printf("PANIC: Cannot find latest data file\r\n");
		for(;;){}
	}
		...
		if(previous_CS && !current_CS){
			...
			case DIRECTORY:
				...
				if( SD_Directory2 == NULL ) {
					...
					strcpy( SD_Directory2, "STM32 Directory Contents\r\n");
				}
				...
				sprintf(SD_space_description,"%lu KB total drive space.\r\n%lu KB available.\r\n",tot_sect / 2, fre_sect / 2);
				strcat( SD_Directory2, SD_space_description);

				/* Get directory contents */
				strcpy(full_path_name, "/");
				directory_lines=3;
				result= get_SD_directory(full_path_name);
				printf("Directory Length %d\r\n", strlen(SD_Directory2));
				...
				printf("Directory Uploaded %d\r\n", strlen(SD_Directory2));
				free(SD_Directory2);
				break;

			case RECORD:
				if(SPI_buffer[6]*256+SPI_buffer[7] ==0){
					sprintf(filenumber,"%04d", ++current_max_filenumber);
				}
				else{
					sprintf(filenumber,"%04d", SPI_buffer[6]*256+SPI_buffer[7]);
				}

				strcpy(filename,"FILE");
				strcat(filename,filenumber);
				strcat(filename,".DAT");
				printf("Record: Sampling %3dkHz  Gain %1d   Duration %4dmS  %12s\r\n", SPI_buffer[1]*256+SPI_buffer[2], SPI_buffer[3], SPI_buffer[4]*256+SPI_buffer[5],filename);

				millisecs_to_record = SPI_buffer[4]*256+SPI_buffer[5];
